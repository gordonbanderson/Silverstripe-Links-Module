...
class LinksFolder extends Page {
	...
	function getCMSFields() {
		...
		$nameField = new TextField('SlotName', _t('Link.SLOT_NAME',
			'Name of the slot on the page where these links will appear'));
		$fields->addFieldToTab('Root.PageSlot', $nameField);
		$fields->renameField( "Content", "Description" );
		$gridConfig = GridFieldConfig_RelationEditor::create()->
			addComponent( new GridFieldSortableRows( 'SortOrder' ) );
		$gridConfig->getComponentByType( 'GridFieldAddExistingAutocompleter' )->
			setSearchFields( array( 'URL', 'Title', 'Description' ) );
		$gridField = new GridField( "Links", "List of Links:", $this->Links()->
			sort( 'SortOrder' ), $gridConfig );
